...
function Gradual({
    ...
    let canvas: CanvasRenderingContext2D & {draw: () => void};
    const destroy = () => {
        if (timer) {
            clearInterval(timer);
            timer = 0;
    ...
    const init = () => {
        ...
        if (typeof canvasDom == 'string') {
            //判断canvas类型
            //此处进行转化
            canvasDom = <HTMLCanvasElement>document.getElementById(canvasDom);
        }
        if (!width) {
        ...
        canvas = <CanvasRenderingContext2D & {draw: () => void}>canvasDom.getContext('2d');
        ...
        timer = <number><unknown>setInterval(() => {
            changeCanvas()
        }, 50)
